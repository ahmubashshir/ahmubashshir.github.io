...
const simpleIcons = Object.
fromEntries(Object.values(require("simple-icons/icons")).map(e => {
	return [e.slug, e];
}).concat([[
	'Get',
	function (name)
	{
		if (name in this && 'svg' in this[name])
			return this[name];
	}
]]));
...
badges.forEach(
	(badge, idx, arr) => {
		...
		badgeInfo.push([badge.name, _badge.info]);
...
fs.writeFileSync(
	path.join(root, 'data/badge.json'),
	JSON.stringify(Object.fromEntries(badgeInfo)),
	'utf8'
);
