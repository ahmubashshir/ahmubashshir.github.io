...
(function () {
	const version =
		'{{ slicestr ((.Lastmod | time.AsTime).Unix | sha1) 0 7 }}';
			...
			.then(cache => {
					return cache.addAll(staticAssets.map(url =>
						new Request(url, {
							...
							Promise.all(staticAssets.map(url => {
	...
	function getCacheForPath(path, domain) {
		if ((staticAssets.includes(path) || path.startsWith('/assets/')) ===
			true) {
	...
	self.addEventListener('fetch', event => {
		...
		if ((url.href.startsWith('{{ $.Site.BaseURL }}') ||
				whitelist.includes(url.hostname)) === false) {
			return;
		}
		...
		event.respondWith(
			...
			.then(response => {
				...
				stashInCache(getCacheForPath(url.pathname
					, url.hostname), request, copy);
				return response;
			})
			.catch((e) => {
				console.log(e)
				// CACHE or FALLBACK
				if (request.headers.get('Accept').indexOf(
						'text/html') !== -1) {
					return caches.match(request)
						.then(response => response || caches
							.match('/offline/'));
				} else {
